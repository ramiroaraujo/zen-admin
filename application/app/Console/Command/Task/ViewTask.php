 ...
 * @var array
 */
	public $scaffoldActions = array('index', 'view', 'add', 'edit', 'delete');
 ...
 */
	protected function _interactive() {
      ...
      $indexFields = $vars['fields'];
			$wannaSetIndexFields = $this->in(__d('cake_console', "Would you like to select the fields for the list columns?"), array('y', 'n'), 'y');
			if ($wannaSetIndexFields == 'y') {
				$indexFields = array();
				$num = 0;
				foreach ($vars['schema'] as $field => $data)
				{
					$default = 'y';
					if ($field == 'id') $default = 'n';
					if ($field == 'name' || $field == 'title') $default = 'y';
          if ($field == 'slug') $default = 'n';
          if ($data['type'] == 'text') $default = 'n';
          if ($num > 7) $default = 'n';
          if ($field == 'modified') $default = 'n';

					$selected = $this->in(__d('cake_console', "List the field") . " " . $field . "?", array('y', 'n'), $default);
					if ($selected == 'y') {
            $num++;
						$indexFields[] = $field;
					}
			...
			$vars['all_fields'] = $vars['fields'];
			$vars['index_fields'] = $indexFields;
        ...
        //<?php //echo $this->Admin->navListLink( 'Examples', array( 'controller' => 'examples', 'action' => 'index' ) );
        //pluralHumanName
        $controllerName = Inflector::tableize($this->controllerName);
        $pattern = "/<!-- nav injects >> -->.+?'controller' ?=> ?'{$controllerName}'.+?<!-- << nav injects -->/ms";
        if ( !preg_match( $pattern, $layoutText ) )
        {
          $pattern = "/(<!-- nav injects >> -->)/ms";
          $replace = "$1\n" . str_repeat( "\t", 8 ) . "<?php echo \$this->Admin->navListLink( '{$vars['pluralHumanName']}', array( 'controller' => '{$controllerName}', 'action' => 'index' ) ); ?>";
 ...
 */
	protected function _loadController() {
		...
		if ($modelObj) {
			...
			$singularHumanName = $modelObj->singularName;
			$schema = $modelObj->schema(true);
			$fields = array_keys($schema);
            $fieldNames = $modelObj->fieldNames;
			$associations = $this->_associations($modelObj);
		} else {
			$primaryKey = $displayField = null;
			$singularVar = Inflector::variable(Inflector::singularize($this->controllerName));
			$singularHumanName = $modelObj->pluralName;
			$fields = $schema = $associations = array();
		}
		$pluralVar = Inflector::variable($this->controllerName);
		$pluralHumanName = $modelObj->pluralName;

		return compact('modelClass', 'schema', 'primaryKey', 'displayField', 'singularVar', 'pluralVar',
				'singularHumanName', 'pluralHumanName', 'fields', 'associations', 'fieldNames');
	}
 ...
 */
	public function bakeActions($actions, $vars) {
      ...
      $vars['fields'] = $action == 'admin_index' ? $vars['index_fields'] : $vars['all_fields'];
	...
	protected function _associations(Model $model) {
		...
		foreach ($keys as $type) {
			foreach ($model->{$type} as $assocKey => $assocData) {
				...
				$associations[$type][$assocKey]['fieldNames'] = $model->{$assocKey}->fieldNames;
				$associations[$type][$assocKey]['singularName'] = $model->{$assocKey}->singularName;
				$associations[$type][$assocKey]['pluralName'] = $model->{$assocKey}->pluralName;
			}
		}
		return $associations;
	}
