...
class AdminAppController extends AppController
{
    ...
    public function admin_search()
    {
        ...
        $this->Paginator->settings[$this->modelClass]['conditions'] = $this->{$this->modelClass}->parseCriteria(
            $this->passedArgs
        );
        $this->set(
            Inflector::variable(Inflector::pluralize($this->modelClass)),
            $this->Paginator->paginate($this->modelClass)
        );
        $this->set('conditions', json_encode($this->Paginator->settings[$this->modelClass]['conditions']));
    ...
    public function admin_add_related($parent, $id)
    {
        ...
        $this->request->data[$this->modelClass] = Hash::check($this->request->data, $this->modelClass) ? Hash::merge(
    ...
    private function generateCsv($data, $noheader, $separator, $filename = 'export', $currentModel = null)
    {
        ...
        if (!$noheader) {
            ...
            if ($currentModel) {
                $fieldnames = array_flip($currentModel->fieldNames);
            }
    ...
    public function admin_bulk_export()
    {
        ...
        if (Hash::check($this->request->data, 'Export.process')) {
            ...
            if (Hash::get($this->request->data, 'Export.type') == 'id') {
                $conditions = array(
                    "$currentModel->name.id" => explode(
                        ',',
                        Hash::get($this->request->data, 'Export.conditions')
                    )
                );
            } else {
                if (Hash::get($this->request->data, 'Export.type') == 'search') {
    ...
    public function admin_bulk_delete()
    {
        ...
        if (Hash::check($this->request->data, 'Delete.process')) {
            ...
            $conditions = array(
                "$currentModel->name.id" => explode(
                    ',',
                    Hash::get($this->request->data, 'Delete.conditions')
                )
            );
            $this->request->data['Delete']['conditions'] = json_encode($conditions);
        }

        $records = $currentModel->find(
            'all',
            array(
